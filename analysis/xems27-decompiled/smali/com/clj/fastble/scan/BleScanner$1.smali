.class Lcom/clj/fastble/scan/BleScanner$1;
.super Lcom/clj/fastble/scan/BleScanPresenter;
.source "BleScanner.java"


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lcom/clj/fastble/scan/BleScanner;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x0
    name = null
.end annotation


# instance fields
.field final synthetic this$0:Lcom/clj/fastble/scan/BleScanner;


# direct methods
.method constructor <init>(Lcom/clj/fastble/scan/BleScanner;)V
    .locals 0
    .param p1, "this$0"    # Lcom/clj/fastble/scan/BleScanner;

    .line 33
    iput-object p1, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    invoke-direct {p0}, Lcom/clj/fastble/scan/BleScanPresenter;-><init>()V

    return-void
.end method


# virtual methods
.method public onLeScan(Lcom/clj/fastble/data/BleDevice;)V
    .locals 1
    .param p1, "bleDevice"    # Lcom/clj/fastble/data/BleDevice;

    .line 45
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->ismNeedConnect()Z

    move-result v0

    if-eqz v0, :cond_1

    .line 46
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    .line 47
    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->getBleScanPresenterImp()Lcom/clj/fastble/callback/BleScanPresenterImp;

    move-result-object v0

    check-cast v0, Lcom/clj/fastble/callback/BleScanAndConnectCallback;

    .line 48
    .local v0, "callback":Lcom/clj/fastble/callback/BleScanAndConnectCallback;
    if-eqz v0, :cond_0

    .line 49
    invoke-virtual {v0, p1}, Lcom/clj/fastble/callback/BleScanAndConnectCallback;->onLeScan(Lcom/clj/fastble/data/BleDevice;)V

    .line 51
    .end local v0    # "callback":Lcom/clj/fastble/callback/BleScanAndConnectCallback;
    :cond_0
    goto :goto_0

    .line 52
    :cond_1
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->getBleScanPresenterImp()Lcom/clj/fastble/callback/BleScanPresenterImp;

    move-result-object v0

    check-cast v0, Lcom/clj/fastble/callback/BleScanCallback;

    .line 53
    .local v0, "callback":Lcom/clj/fastble/callback/BleScanCallback;
    if-eqz v0, :cond_2

    .line 54
    invoke-virtual {v0, p1}, Lcom/clj/fastble/callback/BleScanCallback;->onLeScan(Lcom/clj/fastble/data/BleDevice;)V

    .line 57
    .end local v0    # "callback":Lcom/clj/fastble/callback/BleScanCallback;
    :cond_2
    :goto_0
    return-void
.end method

.method public onScanFinished(Ljava/util/List;)V
    .locals 6
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/List<",
            "Lcom/clj/fastble/data/BleDevice;",
            ">;)V"
        }
    .end annotation

    .line 69
    .local p1, "bleDeviceList":Ljava/util/List;, "Ljava/util/List<Lcom/clj/fastble/data/BleDevice;>;"
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->ismNeedConnect()Z

    move-result v0

    if-eqz v0, :cond_4

    .line 70
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    .line 71
    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->getBleScanPresenterImp()Lcom/clj/fastble/callback/BleScanPresenterImp;

    move-result-object v0

    check-cast v0, Lcom/clj/fastble/callback/BleScanAndConnectCallback;

    .line 72
    .local v0, "callback":Lcom/clj/fastble/callback/BleScanAndConnectCallback;
    if-eqz p1, :cond_2

    invoke-interface {p1}, Ljava/util/List;->size()I

    move-result v1

    const/4 v2, 0x1

    if-ge v1, v2, :cond_0

    goto :goto_0

    .line 77
    :cond_0
    if-eqz v0, :cond_1

    .line 78
    const/4 v1, 0x0

    invoke-interface {p1, v1}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Lcom/clj/fastble/data/BleDevice;

    invoke-virtual {v0, v1}, Lcom/clj/fastble/callback/BleScanAndConnectCallback;->onScanFinished(Lcom/clj/fastble/data/BleDevice;)V

    .line 80
    :cond_1
    move-object v1, p1

    .line 81
    .local v1, "list":Ljava/util/List;, "Ljava/util/List<Lcom/clj/fastble/data/BleDevice;>;"
    new-instance v2, Landroid/os/Handler;

    invoke-static {}, Landroid/os/Looper;->getMainLooper()Landroid/os/Looper;

    move-result-object v3

    invoke-direct {v2, v3}, Landroid/os/Handler;-><init>(Landroid/os/Looper;)V

    new-instance v3, Lcom/clj/fastble/scan/BleScanner$1$1;

    invoke-direct {v3, p0, v1, v0}, Lcom/clj/fastble/scan/BleScanner$1$1;-><init>(Lcom/clj/fastble/scan/BleScanner$1;Ljava/util/List;Lcom/clj/fastble/callback/BleScanAndConnectCallback;)V

    const-wide/16 v4, 0x64

    invoke-virtual {v2, v3, v4, v5}, Landroid/os/Handler;->postDelayed(Ljava/lang/Runnable;J)Z

    goto :goto_1

    .line 73
    .end local v1    # "list":Ljava/util/List;, "Ljava/util/List<Lcom/clj/fastble/data/BleDevice;>;"
    :cond_2
    :goto_0
    if-eqz v0, :cond_3

    .line 74
    const/4 v1, 0x0

    invoke-virtual {v0, v1}, Lcom/clj/fastble/callback/BleScanAndConnectCallback;->onScanFinished(Lcom/clj/fastble/data/BleDevice;)V

    .line 88
    .end local v0    # "callback":Lcom/clj/fastble/callback/BleScanAndConnectCallback;
    :cond_3
    :goto_1
    goto :goto_2

    .line 89
    :cond_4
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->getBleScanPresenterImp()Lcom/clj/fastble/callback/BleScanPresenterImp;

    move-result-object v0

    check-cast v0, Lcom/clj/fastble/callback/BleScanCallback;

    .line 90
    .local v0, "callback":Lcom/clj/fastble/callback/BleScanCallback;
    if-eqz v0, :cond_5

    .line 91
    invoke-virtual {v0, p1}, Lcom/clj/fastble/callback/BleScanCallback;->onScanFinished(Ljava/util/List;)V

    .line 94
    .end local v0    # "callback":Lcom/clj/fastble/callback/BleScanCallback;
    :cond_5
    :goto_2
    return-void
.end method

.method public onScanStarted(Z)V
    .locals 1
    .param p1, "success"    # Z

    .line 37
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->getBleScanPresenterImp()Lcom/clj/fastble/callback/BleScanPresenterImp;

    move-result-object v0

    .line 38
    .local v0, "callback":Lcom/clj/fastble/callback/BleScanPresenterImp;
    if-eqz v0, :cond_0

    .line 39
    invoke-interface {v0, p1}, Lcom/clj/fastble/callback/BleScanPresenterImp;->onScanStarted(Z)V

    .line 41
    :cond_0
    return-void
.end method

.method public onScanning(Lcom/clj/fastble/data/BleDevice;)V
    .locals 1
    .param p1, "result"    # Lcom/clj/fastble/data/BleDevice;

    .line 61
    iget-object v0, p0, Lcom/clj/fastble/scan/BleScanner$1;->this$0:Lcom/clj/fastble/scan/BleScanner;

    invoke-static {v0}, Lcom/clj/fastble/scan/BleScanner;->access$100(Lcom/clj/fastble/scan/BleScanner;)Lcom/clj/fastble/scan/BleScanPresenter;

    move-result-object v0

    invoke-virtual {v0}, Lcom/clj/fastble/scan/BleScanPresenter;->getBleScanPresenterImp()Lcom/clj/fastble/callback/BleScanPresenterImp;

    move-result-object v0

    .line 62
    .local v0, "callback":Lcom/clj/fastble/callback/BleScanPresenterImp;
    if-eqz v0, :cond_0

    .line 63
    invoke-interface {v0, p1}, Lcom/clj/fastble/callback/BleScanPresenterImp;->onScanning(Lcom/clj/fastble/data/BleDevice;)V

    .line 65
    :cond_0
    return-void
.end method
