...
def render(running: bool) -> Image.Image:
    ...
    f_hint = font(18)
    f_chip = font(22, True)
    clock = "0:42" if running else "--:--"
    tw = d.textlength(clock, font=f_clk)
    d.text(((W - tw) / 2, 36), clock, fill="#AEAEB2", font=f_clk)
    hr = "88"
    tw = d.textlength(hr, font=f_hr)
    d.text(((W - tw) / 2, 66), hr, fill="#FFD60A", font=f_hr)
    sub = "Z3 · лимит 170"
    tw = d.textlength(sub, font=f_sub)
    d.text(((W - tw) / 2, 128), sub, fill="#C7C7CC", font=f_sub)
    zones = ["#5AC8FA", "#30D158", "#FFD60A", "#FF9F0A", "#FF453A"]
    zx = (W - (32 * 5 + 6 * 4)) // 2
    for i, col in enumerate(zones):
        on = i < 3
        d.rounded_rectangle((zx + i * 38, 156, zx + i * 38 + 32, 164), 4, fill=col if on else "#3A3A3C")
    col = "#FF9F0A" if running else "#30D158"
    d.rounded_rectangle((PLAY_X, PLAY_Y, PLAY_X + PLAY, PLAY_Y + PLAY), 70, fill=col)
    paste_icon(im, "pause.png" if running else "play.png", PLAY_X + (PLAY - 64) // 2, PLAY_Y + (PLAY - 64) // 2, 64)
    hint = "музика · ▶" if running else "▶ старт"
    tw = d.textlength(hint, font=f_hint)
    d.text(((W - tw) / 2, PLAY_Y + PLAY + 8), hint, fill="#8E8E93", font=f_hint)
    chips = [("Старт", running), ("AI", False), ("Таймер", False), ("Пулс", False)]
    cy = PLAY_Y + PLAY + 8 + 22 + 8
    for i, (name, on) in enumerate(chips):
        row, col_i = divmod(i, 2)
        x = 10 + col_i * 100
        y = cy + row * 64
        d.rounded_rectangle((x, y, x + 92, y + 56), 18, fill="#1C1C1E", outline="#30D158" if on else "#2C2C2E", width=2)
        tw = d.textlength(name, font=f_chip)
        d.text((x + (92 - tw) / 2, y + 14), name, fill="#FFFFFF", font=f_chip)
    return im
