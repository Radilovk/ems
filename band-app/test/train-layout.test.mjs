...
ok(ux.match(/\.tr-play-glow\s*\{[^}]*position:\s*absolute/s), 'play glow out of flow (stack must not grow)')
ok(!ux.match(/\.tr-play-row\s*\{[^}]*flex-direction:\s*column/s), 'play row stack overlays children')
ok(ux.includes('stop-mark'), 'stop uses CSS square (not PNG image)')
ok(ux.match(/\.stop-core\s*\{[^}]*position:\s*absolute/s), 'stop core centered in ring via absolute')
ok(ux.match(/\.stop-ring\s*\{[^}]*position:\s*absolute/s), 'stop ring out of stack flow')
ok(!ux.includes('stop-img'), 'no stop.png image on train (Vela mis-centers icons)')
