...
Версия на кода: **v1.1.55-ble** · Потвърдено на: Huawei P60 Pro + Xiaomi Smart Band 8 (fw 2.3.14)
...
## 9a. Интерфейс (от v1.1.55-ble)

Кодът е в `wearable/WearableSyncHelper.java`, `WearableLivePanel.java`, `WearableBandPicker.java` и
`WearableUi.java`. Новите елементи се създават в Java код, без нови ресурсни ID. Текстовете са на
български, с английски при друг език на системата.

| Елемент | Какво показва / прави |
|---|---|
| **Кръг с пулса** | Голямо число; цвят по зона спрямо „Праг пулс“ (Z1 сиво <60 %, Z2 зелено 60–70 %, Z3 жълто 70–80 %, Z4 оранжево 80–90 %, Z5 червено ≥90 %); под числото „уд/мин · Z3 / Аеробна“. Статусът е на човешки език („Свързване…“, „Мери пулс…“, „Връзката прекъсна — свързва отново“, „Грешен auth key“ …). Ако последното измерване е по-старо от 10 s, числото посивява и се показва „преди N s“. Обновява се всяка секунда. |
| Бутон **i** на кръга | Отваря панела „Данни от гривната“ |
| **Данни от гривната** | Пулс и зона, стъпки, калории, поле 3 (активност), поле 5, събития/сек, възраст на последното събитие, брой събития. Обновява се всяка секунда. Бутони: **Сподели записа** (изпраща band-realtime.csv и band-raw.csv като текст през всяко приложение), **Технически лог**, **Затвори** |
| Настройки → **Избери** до MAC | Списък на сдвоените Bluetooth устройства, гривните Xiaomi са най-отгоре. Без сканиране и без разрешение за местоположение |
| Настройки → auth key | Става зелен при 32 валидни hex символа и червен при грешен формат |
| Настройки → статус | Разбираем статус вместо build tag и вътрешни състояния |
| Настройки → **i** | Помощ стъпка по стъпка (зони, авто-намаляване, какво да се спре) |
