...
# Match card root; height may be 170 or 190 after theme/branding scripts.
CARD_ROOT_RE = re.compile(
    r"<LinearLayout android:orientation=\"horizontal\" android:background=\"@drawable/ui_card_background\" "
    r'android:layout_width="fill_parent" android:layout_height="(\d+\.0dip)" '
    r'android:layout_margin="@dimen/ui_card_margin"\s*'
    r'xmlns:android="http://schemas.android.com/apk/res/android" xmlns:app="http://schemas.android.com/apk/res-auto">\s*'
    r'(<LinearLayout android:orientation="horizontal" android:layout_width="fill_parent" '
    r'android:layout_height="fill_parent">)',
    re.DOTALL,
)

ADD_ROW_OVERLAY = f"""
        <LinearLayout android:id="@id/trainAddParticipantWrap" android:gravity="center_horizontal" android:orientation="vertical" android:layout_width="wrap_content" android:layout_height="wrap_content" android:layout_alignRight="@id/trainUserCard" android:layout_alignBottom="@id/trainUserCard" android:layout_marginRight="8.0dip" android:layout_marginBottom="-36.0dip" android:visibility="gone">
...
def patch_user_item_layout(path: Path) -> None:
    ...
    # Undo PR225 broken root swap (RelativeLayout root) if present from older builds.
    if '@id/trainUserCard"' in text and 'android:orientation="vertical"' not in text.split("\n", 1)[0]:
        print(f"{path.name}: skipped (legacy RelativeLayout root — rebuild from fresh decompile)")
        return

    match = CARD_ROOT_RE.search(text)
    if not match:
        print(f"{path.name}: skipped (unexpected layout shape)")
        return

    card_height = match.group(1)
    inner_open = match.group(2)
    replacement = (
        '<LinearLayout android:orientation="vertical" android:layout_width="fill_parent" '
        'android:layout_height="wrap_content" android:clipToPadding="false" android:paddingBottom="42.0dip"\n'
        '  xmlns:android="http://schemas.android.com/apk/res/android" xmlns:app="http://schemas.android.com/apk/res-auto">\n'
        '    <RelativeLayout android:layout_width="fill_parent" android:layout_height="wrap_content" '
        'android:clipToPadding="false">\n'
        f'        <LinearLayout android:id="@id/trainUserCard" android:orientation="horizontal" '
        f'android:background="@drawable/ui_card_background" android:layout_width="fill_parent" '
        f'android:layout_height="{card_height}" android:layout_margin="@dimen/ui_card_margin">\n'
        f"            {inner_open}"
    )
    text = CARD_ROOT_RE.sub(replacement, text, count=1)

    closing_tail = "    </LinearLayout>\n</LinearLayout>"
    if not text.rstrip().endswith(closing_tail):
        raise RuntimeError(f"{path.name}: unexpected layout ending")
    text = text.rstrip()[: -len(closing_tail)] + (
        "    </LinearLayout>\n        </LinearLayout>\n" + ADD_ROW_OVERLAY.strip() + "\n"
    )
    path.write_text(text, encoding="utf-8")
    print(f"{path.name}: add-user overlay at slot bottom-right (LinearLayout root preserved)")
...
def revert_footer_smali() -> None:
    """Remove footer-row experiment; restore clean bind() for real users."""
...
""",
        ...
        ad = ad.replace(".field private static final VIEW_TYPE_ADD_FOOTER:I = 0x2\n\n", "")
    ...
    if any(marker in vh for marker in FOOTER_VH_MARKERS):
        vh = vh.replace("\n.field private isFooter:Z\n", "\n")
        vh = re.sub(
            r"\.method private bindFooterAddListener\(Landroid/view/View;\)V\n.*?\.end method\n\n?",
            "",
            vh,
            flags=re.DOTALL,
        )
    ...
    if-eqz p1, :cond_0""",
        ...
        vh = re.sub(
            ...
            r"    if-(eq|ne)z v0, :cond_footer_done\n\n",
            "",
            vh,
        )
        vh = vh.replace("    :cond_footer_done\n", "")
        TRAIN_VIEW_HOLDER.write_text(vh, encoding="utf-8")
        print("TrainViewHolder: reverted footer bind guard (fixes Chinese placeholder)")

    for footer_layout in (
        RES / "layout/train_add_participant_footer.xml",
        RES / "layout-night/train_add_participant_footer.xml",
    ):
        if footer_layout.is_file():
            footer_layout.unlink()
            print(f"removed {footer_layout.name}")
...
def patch_train_adapter() -> None:
    ...
    if ":cond_4" in text and "updateAddParticipantButton(Z)V" in text:
        ...
        return
    if ADAPTER_ON_BIND in text:
        text = text.replace(ADAPTER_ON_BIND, ADAPTER_ON_BIND_LAST_ROW, 1)
    else:
        text, count = re.subn(
            r"\.method public onBindViewHolder\(Lcom/isaigu/gymapp/train/TrainViewHolder;I\)V\n.*?\.end method\n",
            ADAPTER_ON_BIND_LAST_ROW,
            text,
            count=1,
            flags=re.DOTALL,
        )
        if count == 0 or "updateAddParticipantButton(Z)V" not in text:
