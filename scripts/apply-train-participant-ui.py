...
# PR225 bug: if-eqz skipped bindNotEmpty() for real users — use if-nez (skip only footer rows).
...
def strip_row_overlay_patches() -> None:
    """Remove PR225 row-overlay smali before applying PR224 footer approach."""
    ...
    if "updateAddParticipantButton(Z)V" in ad:
        ...
        print("TrainAdapter: removed PR225 row-overlay bind patch")

    vh = TRAIN_VIEW_HOLDER.read_text(encoding="utf-8")
    changed = False
    if "addParticipantWrap:Landroid/view/View;" in vh:
        vh = vh.replace(
    ...
    if changed:
        ...
        print("TrainViewHolder: removed PR225 row-overlay wiring")
...
def cleanup_old_row_button_patches() -> None:
    ...
    if VH_CONSTRUCTOR_MARKER in vh and "bindFooterAddListener" not in vh:
        ...
        elif "if-eqz v0, :cond_footer_done" in vh:
            vh = vh.replace("if-eqz v0, :cond_footer_done", "if-nez v0, :cond_footer_done", 1)
            print("TrainViewHolder: fixed inverted isFooter bind guard (PR225)")
...
def main() -> int:
    ...
    print("Train participant UI patches applied (PR224 footer + PR225 bind fix).")
    return 0
