#!/usr/bin/env bash
# Compile AvatarStageLayout.java to smali (prebuilt fallback when SDK missing).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
JAVA_SRC="${ROOT}/branding/java/src/com/isaigu/gymapp/widget/AvatarStageLayout.java"
OUT_DIR="${ROOT}/build/avatar-stage-java"
...
PREBUILT="${BRANDING_SMALI}/AvatarStageLayout.smali"
...
if [[ ! -f "${ANDROID_JAR}" ]]; then
  ...
  echo "ERROR: Android SDK missing (${ANDROID_JAR}) and no prebuilt AvatarStageLayout.smali"
  exit 1
fi

echo "Compiling AvatarStageLayout..."
rm -rf "${CLASSES_DIR}"
mkdir -p "${CLASSES_DIR}"
...
(
  ...
  "${D8}" --min-api 21 --lib "${ANDROID_JAR}" --output "${OUT_DIR}/dex" \
    com/isaigu/gymapp/widget/AvatarStageLayout.class
)
...
install -m 0644 "${SMALI_OUT}/com/isaigu/gymapp/widget/AvatarStageLayout.smali" "${PREBUILT}"
