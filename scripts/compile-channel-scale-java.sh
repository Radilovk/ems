#!/usr/bin/env bash
# Compile ChannelStrengthScale.java to smali (prebuilt fallback when SDK missing).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
JAVA_SRC="${ROOT}/branding/java/src/com/isaigu/gymapp/train/utils/ChannelStrengthScale.java"
OUT_DIR="${ROOT}/build/channel-scale-java"
...
PREBUILT="${BRANDING_SMALI}/ChannelStrengthScale.smali"
...
if [[ ! -f "${ANDROID_JAR}" ]]; then
  ...
  echo "ERROR: Android SDK missing (${ANDROID_JAR}) and no prebuilt ChannelStrengthScale.smali"
  exit 1
fi
...
(
  ...
  "${D8}" --min-api 21 --lib "${ANDROID_JAR}" --output "${OUT_DIR}/dex" \
    com/isaigu/gymapp/train/utils/ChannelStrengthScale.class
)
...
install -m 0644 \
  "${SMALI_OUT}/com/isaigu/gymapp/train/utils/ChannelStrengthScale.smali" \
  "${PREBUILT}"
